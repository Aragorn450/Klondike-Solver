...
struct HashKey {
  ...
  bool operator==(HashKey const &key) const {
    int i = 0;
    while (i < KEYSIZEM1 && Key[i] == key.Key[i]) {
      i++;
    }
    return Key[i] == key.Key[i];
  }
  char &operator[](int index) { return Key[index]; }
};
...
template <typename T>
KeyValue<T>::KeyValue(int hash, HashKey const &key, T const &value,
                      KeyValue<T> *next) {
  Next = next;
  Key = key;
  Hash = hash;
...
template <typename T>
HashMap<T>::HashMap(int powerOf2) {
...
template <typename T>
int HashMap<T>::MaxLength() {
  return maxLength;
}
template <typename T>
int HashMap<T>::SlotsUsed() {
  return slotsUsed;
}
template <typename T>
int HashMap<T>::Capacity() {
  return capacity + 1;
}
template <typename T>
KeyValue<T> *HashMap<T>::Add(HashKey const &key, T const &value) {
  ...
  if (node->Key[0] != 0) {
    node->Next =
        new KeyValue<T>(node->Hash, node->Key, node->Value, node->Next);
  }
  node->Hash = hash;
  node->Key = key;
  node->Value = value;
