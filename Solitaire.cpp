...
void SolitaireWorker::RunMinimalWorker(void *closedPointer) {
  ...
  while (closed.Size() < maxClosedCount && doneCount > 0) {
    ...
    if (s.FoundationCount() > maxFoundationCount ||
        (s.FoundationCount() == maxFoundationCount &&
...
SolveResult SolitaireWorker::Run(int numThreads) {
  ...
  startMoves =
      solitaire->MinimumMovesLeft() + solitaire->MovesMadeNormalizedCount();
  int powerOf2 = 1;
  ...
  shared_ptr<MoveNode> firstNode =
      solitaire->MovesMadeCount() > 0
          ? make_shared<MoveNode>(
                solitaire->GetMoveMade(solitaire->MovesMadeCount() - 1))
          : NULL;
  ...
  for (int i = 0; i < numThreads; i++) {
    threads[i] =
        thread(&SolitaireWorker::RunMinimalWorker, this, (void *)closed);
  ...
  SolveResult result =
      closed->Size() >= maxClosedCount
          ? (maxFoundationCount == 52 ? SolvedMayNotBeMinimal
                                      : CouldNotComplete)
          : (maxFoundationCount == 52 ? SolvedMinimal : Impossible);
  delete closed;
  return result;
}

SolveResult Solitaire::SolveRandom(int numberOfTimesToPlay,
                                   int solutionsToFind) {
  MakeAutoMoves();
  ...
  shared_ptr<MoveNode> firstNode =
      movesMadeCount > 0 ? make_shared<MoveNode>(movesMade[0]) : NULL;
  shared_ptr<MoveNode> node = firstNode;
  ...
  int solutionCount = 0;
  while ((maxFoundationCount < 52 || solutionCount < solutionsToFind) &&
         numberOfTimesToPlay-- > 0) {
    while (foundationCount < 52 && movesAvailableCount > 0 &&
           movesMadeCount < 500 && roundCount < maxRoundCount) {
      int drawIndex = -1;
      for (int i = 0; i < movesAvailableCount; i++) {
      ...
      if (drawIndex <= 0) {
        ...
        int r = random.Next1() %
                (drawIndex + (random.Next1() & 1) + (movesAvailableCount >> 3));
        if (r >= drawIndex) {
          MakeMove((random.Next1() % (movesAvailableCount - drawIndex)) +
                   drawIndex);
        } else {
          MakeMove(r);
        }
...
SolveResult Solitaire::SolveFast(int maxClosedCount, int twoShift,
                                 int threeShift) {
  MakeAutoMoves();
  if (movesAvailableCount == 0) {
    return foundationCount == 52 ? SolvedMinimal : Impossible;
  ...
  shared_ptr<MoveNode> firstNode =
      movesMadeCount > 0 ? make_shared<MoveNode>(movesMade[movesMadeCount - 1])
                         : NULL;
  shared_ptr<MoveNode> node = firstNode;
  ...
  while (closed.Size() < maxClosedCount) {
    ...
    // Check for best solution to foundations
    if (foundationCount > maxFoundationCount ||
        (foundationCount == maxFoundationCount &&
         bestSolutionMoveCount > movesTotal)) {
      bestSolutionMoveCount = movesTotal;
      maxFoundationCount = foundationCount;
    ...
    int bestMoveAdded1 = 512, bestMoveHelper1 = 512, bestMoveAdded2 = 512,
        bestMoveHelper2 = 512, bestMoveAdded3 = 512, bestMoveHelper3 = 512;
    for (int i = 0; i < movesAvailableCount; i++) {
...
SolveResult Solitaire::SolveMinimalMultithreaded(int numThreads,
                                                 int maxClosedCount) {
...
int Solitaire::GetTalonCards(Card talon[], int talonMoves[]) {
  ...
  for (int j = (stockSize > 0 && stockSize - drawCount <= 0)
                   ? 0
                   : stockSize - drawCount;
       j >= 0; j -= drawCount) {
  ...
  // Check cards already turned over in the waste, meaning we have to "redeal"
  // the deck to get to it
  ...
  // Check cards in stock after a "redeal". Only happens when draw count > 1 and
  // you have access to more cards in the talon
  if (lastIndex > wasteSize && wasteSize > -1) {
    amountToDraw += wasteSize;
    amountToDraw += stockSize;
    for (int j = (stockSize > 0 && stockSize - lastIndex + wasteSize <= 0)
                     ? 0
                     : stockSize - lastIndex + wasteSize;
         j > 0; j -= drawCount) {
      talon[index] = stock.Up(j);
...
void Solitaire::UpdateAvailableMoves() {
  ...
  if (lastMove.From >= FOUNDATION1C && lastMove.From <= FOUNDATION4H) {
    ...
    for (int i = TABLEAU1; i <= TABLEAU7; ++i) {
      ...
      // logic used to determine if a pile of cards can be moved ontop of
      // another pile of cards
      if (card1.Rank >= card3.Rank || card2.Rank + 1 < card3.Rank ||
      ...
      if (pile1Moved == pile1Length) {  // we are moving all face up cards
        movesAvailable[movesAvailableCount++].Set(
            i, j, pile1Moved, pile1Size > pile1Moved ? 1 : 0);
        continue;
      }

      // look to see if we are covering a card that can be moved to the
      // foundation
    ...
    for (int j = 0; j < talonCount; j++) {
        ...
        if (talonCard.Rank <= foundationMin + 1) {
          if (drawCount == 1) {
            if (cardsToDraw == 0 || movesAvailableCount == 0) {
              ...
              movesAvailable[movesAvailableCount++].Set(WASTE, foundation, 1,
                                                        cardsToDraw);
              break;
            }
          } else {
      ...
      if (pile.Size() != 0) {
        ...
        if (tableauCard.Rank - talonCard.Rank != 1 ||
    ...
    for (int i = FOUNDATION1C; i <= FOUNDATION4H; ++i) {
      ...
      for (int j = TABLEAU1; j <= TABLEAU7; ++j) {
        ...
        if (pile2.Size() != 0) {
          ...
          if ((card.Foundation & 1) == (i & 1) ||
              card.Rank - foundationRank != 1) {
            continue;
          }
  ...
  for (int i = TABLEAU1; i <= TABLEAU7; ++i) {
    ...
    if (card1.Rank - piles[cardFoundation].Size() == 1) {
      ...
      if (card1.Rank <= foundationMin + 1) {
        movesAvailable[0].Set(i, cardFoundation, 1,
      ...
      if (lastMove.From != cardFoundation && lastMove.To != i) {
        movesAvailable[movesAvailableCount++].Set(
            i, cardFoundation, 1, pile1UpSize == 1 && pile1Size > 1 ? 1 : 0);
      }
    }
  ...
  for (int i = FOUNDATION1C; i <= FOUNDATION4H; ++i) {
    Pile &pile1 = piles[i];
    int foundationRank = pile1.Size();
    if (foundationRank == 0 || foundationRank <= foundationMin + 1) {
      continue;
    }
...
void Solitaire::ResetGame() { ResetGame(drawCount, maxRoundCount); }
...
void Solitaire::MakeMove(int index) { MakeMove(movesAvailable[index]); }
...
int Solitaire::FoundationMin() {
  ...
  return blackFoundationMin <= redFoundationMin ? blackFoundationMin
                                                : redFoundationMin;
}
Move Solitaire::GetMoveAvailable(int index) { return movesAvailable[index]; }
Move Solitaire::GetMoveMade(int index) { return movesMade[index]; }
void sample(Random &random, int initialValues[], int targetPositions[],
            int targetCount, int sampleCount, int values[]) {
  int tempValues[52];
  for (int i = 0; i < 52; i++) {
...
bool Solitaire::sampleGames(const int initialValues[], int sampleCount,
                            int values[], int dealNumber) {
  int openValues[52];
...
bool Solitaire::LoadSolitaire(string const &cardSet) {
  ...
  if (cardSet.at(0) - 48 == 0 || cardSet.at(0) - 48 == 1) {
    if (cardSet.size() > 156) {
      return false;
    }
    for (int i = 0; i < 52; i++) {
      ...
      int rank = (cardSet.at(i * 3) - 48) * 10 + cardSet.at(i * 3 + 1) - 48;
...
bool Solitaire::LoadPysol(string const &cardSet) {
  ...
  const int order[28] = {0,  1, 7,  2,  8,  13, 3,  9, 14, 18, 4,  10, 15, 19,
                         22, 5, 11, 16, 20, 23, 25, 6, 12, 17, 21, 24, 26, 27};
  for (int i = 0; i < 28; i++) {
    while (j < cardSet.size() &&
           (cardSet[j] == '\r' || cardSet[j] == '\n' || cardSet[j] == '\t' ||
            cardSet[j] == ' ' || cardSet[j] == ':' || cardSet[j] == '<')) {
      j++;
    }
    if (j + 1 >= cardSet.size()) {
      return false;
...
void Solitaire::SetDrawCount(int drawCount) { this->drawCount = drawCount; }
HashKey Solitaire::GameState() {
  int order[7] = {TABLEAU1, TABLEAU2, TABLEAU3, TABLEAU4,
  ...
  while (current < 7) {
    ...
    do {
      if (piles[order[search - 1]].HighValue() <=
          piles[order[search]].HighValue()) {
        break;
      }
  ...
  int z = 0;
  key[z++] =
      (piles[FOUNDATION1C].Size() << 4) | (piles[FOUNDATION2D].Size() + 1);
...
string Solitaire::GetMoveInfo(Move move) {
  ...
  if (move.Extra > 0) {
    if (move.From != WASTE) {
      if (move.Count > 1) {
        ss << "Move " << (int)move.Count << " cards from tableau "
           << (int)move.From << " on to tableau " << (int)move.To;
      } else {
        fromRank = RANKS[piles[move.From].Low().Rank];
        fromSuit = SUITS[piles[move.From].Low().Suit];
        ss << "Move " << fromRank << fromSuit << " from "
           << (move.From == WASTE
                   ? "waste"
                   : (move.From >= FOUNDATION1C ? "foundation" : "tableau "));
        if (move.From >= TABLEAU1 && move.From <= TABLEAU7) {
          ss << (int)move.From;
        }
        ss << " on to "
           << (move.To >= FOUNDATION1C ? "foundation" : "tableau ");
      ...
      if (move.Extra <= stockSize) {
        drawAmount =
            move.Extra / drawCount + ((move.Extra % drawCount) == 0 ? 0 : 1);
        ...
        drawAmount =
            drawAmount / drawCount + ((drawAmount % drawCount) == 0 ? 0 : 1);
        drawAmount +=
            stockSize / drawCount + ((stockSize % drawCount) == 0 ? 0 : 1);

        int cardsToMove =
            stockSize + stockSize + wasteSize + wasteSize - move.Extra;
        if (cardsToMove > 0) {
      ...
      ss << "Draw " << drawAmount << (drawAmount == 1 ? " time " : " times ")
         << "and move " << fromRank << fromSuit << " from waste on to "
         << (move.To >= FOUNDATION1C ? "foundation" : "tableau ");
      if (move.To >= TABLEAU1 && move.To <= TABLEAU7) {
        ss << (int)move.To;
      }
    ...
    ss << "Move " << (int)move.Count << " cards from tableau " << (int)move.From
       << " on to tableau " << (int)move.To;
  } else {
    fromRank = RANKS[piles[move.From].Low().Rank];
    fromSuit = SUITS[piles[move.From].Low().Suit];
    ss << "Move " << fromRank << fromSuit << " from "
...
void AddMove(stringstream &ss, Move m, int stockSize, int wasteSize,
             int drawCount, bool combine) {
  if (m.Extra > 0) {
    if (m.From != WASTE) {
      if (m.Count > 1) {
        if (!combine) {
          ss << PILES[m.From] << PILES[m.To] << '-' << (int)m.Count << " F"
             << (int)m.From << ' ';
        } else {
      ...
      if (m.Extra <= stockSize) {
        ss << "DR"
           << (m.Extra / drawCount + ((m.Extra % drawCount) == 0 ? 0 : 1))
           << ' ' << PILES[m.From] << PILES[m.To] << ' ';
      } else {
        int temp =
            stockSize / drawCount + ((stockSize % drawCount) == 0 ? 0 : 1);
        if (temp != 0) {
...
string Solitaire::MovesAvailable() {
  // Returns moves available for the current state. Flip moves are combined with
  // the move that caused it in []. See below for move representation.
  stringstream ss;
  for (int i = 0; i < movesAvailableCount; i++) {
...
string Solitaire::MovesMade() {
  ...
  // DR# is a draw move that is done # number of times. ie) DR2 means draw
  // twice, if draw count > 1 it is still DR2.  NEW is to represent the moving
  // of cards from the Waste pile back to the stock pile. A New round.  F# means
  // to flip the card on tableau pile #.  XY means to move the top card from
  // pile X to pile Y.  X will be 1 through 7, W for Waste, or a foundation suit
  // character. 'C'lubs, 'D'iamonds, 'S'pades, 'H'earts  Y will be 1 through 7
  // or the foundation suit character.  XY-# is the same as above except you are
  // moving # number of cards from X to Y.
  stringstream ss;
...
int Solitaire::MovesAvailableCount() { return movesAvailableCount; }
...
int Solitaire::MovesMadeCount() { return movesMadeCount; }
int Solitaire::FoundationCount() { return foundationCount; }
int Solitaire::RoundCount() { return roundCount; }
int Solitaire::DrawCount() { return drawCount; }
...
Move Solitaire::operator[](int index) { return movesMade[index]; }
int Solitaire::MoveValueCount() { return moveValueCount; }
MoveValue Solitaire::GetMoveValue(int i) { return moveValue[i]; }
